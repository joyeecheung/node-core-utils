  ...
  async getResults(data) {
    ...
    const promises = builds.failed.map(({ jobName, buildNumber, url }) => {
      ...
      if (jobName.includes('fanned')) {
        ...
        build = new FannedBuild(cli, request, jobName, buildNumber, isResumed);
      } else if (jobName.includes('linter')) {
        build = new LinterBuild(cli, request, jobName, buildNumber);
      } else if (jobName.includes('freestyle')) {
        build = new TestRun(cli, request, url);
      } else {
        build = new NormalBuild(cli, request, jobName, buildNumber);
      }
      build.full = this.full;
